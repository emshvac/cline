...
export class DiffViewProvider {
    ...
    setLargeFileWriter(writer: LargeFileWriter) {
        this.largeFileWriter = writer
    }
    ...
    async update(accumulatedContent: string, isFinal: boolean, isLargeFile: boolean = false) {
        if (!this.relPath || !this.activeLineController || !this.fadedOverlayController) {
            throw new Error("Required values not set")
        }

    // For large files, we need to handle the content differently
    if (isLargeFile) {
    ...
    // Regular file handling (non-large files)
    this.newContent = accumulatedContent
    const accumulatedLines = accumulatedContent.split("\n")
    if (!isFinal) {
